...
public class BookCUDServiceV1 {
	...
	public void updateBook() {
		...
		String strCode = scanner.nextLine();
		if (strCode.contentEquals("-Q"))
			; // 입력을 끝내기

		// 키보드로 입력받은 코드에 해당하는 도서정보를 가져오기
		BookDTO bookDTO = bookDao.findById(strCode);
		/*
		 * 보통의 경우 PK를 수정하는 프로세스는 좋지 않다. PK를 수정해야 할 경우는 기존의 Data를 삭제하고 새로운 데이터를 INSERT하거나
		...
		if (strName.trim().length() > 0) { // 객체의 chaining 방법
		...
		// 만약 값을 입력하지 않고 Enter을 누르면
		// NumberFormatException이 발생하는 것을
		// 역이용하여 만약 값을 입력하면 해당값을 DTO에 저장하고
		// 그렇지 않으면 건너뛰도록 코딩을 한다
		try {
		...
		// 여기 도착했을 때 bookDTO에 담긴 값은
		// 처음에 테이블에서 읽은 값이 저장되어 있거나
		// 중간에 keyboard로 입력한 값으로 변경되었거나 한다.
